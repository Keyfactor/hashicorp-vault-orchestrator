...
* **added:** `PrivateKeyPath` custom field to HCVKVPEM, pointing to the secret holding the PEM-encoded private key. Optional — omit it for certificate-only / CA trust chain stores that have no private key. Unlike the other Key-Value store types' `PassphrasePath`, no sibling-secret convention is assumed when omitted. Named differently from `PassphrasePath` since it holds the private key itself, not a passphrase.
...
* **fix:** Management-Add on HCVKVPEM now fails clearly when the certificate being added includes a private key but no `PrivateKeyPath` is configured, instead of silently dropping the key.
